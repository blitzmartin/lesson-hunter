  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {courses.map((c) => (
          <Link key={c.id} to={`/courses/${c.id}`} className="bg-paper border border-line p-8 block">
            <span className="font-mono uppercase tracking-widest text-xs text-muted-2">{c.level}</span>
            <h3 className="font-display font-light uppercase tracking-tight text-2xl leading-[0.95] mt-2">
              {c.topic}
            </h3>
