...
function TestResult({
  ...
  return (
    <p
      className={`font-mono text-xs mt-2 ${result.ok ? "text-ink" : "text-muted"}`}
    >
      {result.ok ? "✓ " : "✕ "}
      {result.message}
    </p>
...
export default function Setup() {
  ...
  return (
    <div className="max-w-2xl space-y-16">
      <div>
        <div className="font-mono uppercase tracking-widest text-sm text-muted-2 flex items-center justify-between gap-4">
        ...
        <p className="text-muted mt-4">
          Choose one provider to use across the whole app. Local models via
          Ollama need no API key but may be weaker at video-matching reasoning
          than cloud models.
        </p>

        <div className="mt-6 flex flex-wrap gap-3">
          {(
            ["ollama", ...CLOUD_PROVIDERS.map((p) => p.id)] as LlmProvider[]
          ).map((p) => (
            <button
              key={p}
            ...
            >
              {p === "ollama"
                ? "Ollama (local)"
                : CLOUD_PROVIDERS.find((c) => c.id === p)?.label}
            </button>
          ))}
        </div>
        ...
        <div className="mt-6 border border-line bg-paper-2 px-4 py-3">
          ...
          <p className="text-muted text-sm">
            The model you pick here drives both steps of course generation —
            designing the syllabus and matching each sub-topic to a video — so
            it has a real effect on the result. Larger cloud models (OpenAI,
            Anthropic, Gemini) are generally more reliable at following
            instructions and producing well-formed output. Smaller local Ollama
            models are free and fully private, but may build weaker syllabi or
            occasionally return malformed responses that fail generation. If a
            course comes out shallow, oddly ordered, or generation errors out,
            try a stronger model before assuming something's broken.
          </p>
        </div>
        ...
        {config.llmProvider &&
          CLOUD_PROVIDERS.some((c) => c.id === config.llmProvider) && (
            <div className="mt-8 bg-paper border border-line p-8 space-y-4">
              ...
              <KeyField
                ...
                onChange={(value) =>
                  setKeyInputs((prev) => ({
                    ...prev,
                    [config.llmProvider as string]: value,
                  }))
                }
              ...
              {!config.secretsAvailable && (
                <p className="font-mono text-xs text-muted">
                  Secure key storage isn't available on this system — keys can't
                  be saved safely here.
                </p>
              )}
            </div>
          )}
      ...
      <div>
        ...
        <p className="text-muted mt-4">
          Each sub-topic runs two search calls (200 quota units total) against a
          10,000/day default budget — generating many or large courses in one
          day can hit that limit. That's expected, not a bug.{" "}
          <a
        ...
        <div className="mt-6 bg-paper border border-line p-8 space-y-4">
          <KeyField
            ...
            onChange={(value) =>
              setKeyInputs((prev) => ({ ...prev, youtube: value }))
            }
      ...
      <div>
        ...
        {browser === "chromium" && (
          <p className="text-muted mt-4">
            You're on a Chromium-based browser. Ad-free playback for YouTube
            Premium works inside embedded videos here{" "}
            <strong>if third-party cookies are allowed</strong> for youtube.com
            — check chrome://settings/content/cookies if ads show up despite
            Premium.
          </p>
        )}
        {(browser === "safari" || browser === "firefox") && (
          <p className="text-muted mt-4">
            ...
            ), so ad-free embedded playback is <strong>not supported</strong>{" "}
            here — embedded videos will show standard YouTube ads even with
            Premium.
          </p>
        )}
        {browser === "other" && (
          <p className="text-muted mt-4">
            We couldn't confidently detect your browser. Ad-free embedded
            playback depends on your browser's third-party cookie policy for
            youtube.com.
          </p>
        )}
        <p className="text-muted mt-2">
          Every video also has an <strong>"Open on YouTube"</strong> button that
          opens the real youtube.com page in a new tab — this always guarantees
          ad-free Premium playback, at the cost of leaving the in-app
          experience.
        </p>
      </div>
